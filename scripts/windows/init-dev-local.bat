...
REM ============================================================
REM  OnlyFix - Windows Developer Init LOCAL (Transactional)
...
echo.
echo ======================================================
echo    OnlyFix - Developer Environment Init LOCAL [Windows]
...
REM -- Step 7: Wait for MySQL readiness
...
REM -- Step 8: Composer install
echo [STEP] Installing Composer dependencies...
%COMPOSE_CMD% %CF% exec -T app composer install --no-interaction --optimize-autoloader
if %errorlevel% neq 0 (
...
REM -- Step 10: Run migrations and seeders
echo [STEP] Running database migrations and seeders...
%COMPOSE_CMD% %CF% exec -T app php artisan migrate:fresh --seed --force --no-interaction
if %errorlevel% neq 0 (
    echo [ERROR] Database migration/seeding failed.
    popd
    goto :rollback
)
echo [  OK] Database migrated and seeded.

REM -- Step 11: Storage link
...
REM -- Step 12: Wayfinder route generation
...
REM -- Step 13: Restart node container (triggers npm install + vite)
echo [STEP] Restarting Node container for npm install and Vite dev...
%COMPOSE_CMD% %CF% restart node
if %errorlevel% neq 0 (
...
echo.
echo ======================================================
echo    [OK] OnlyFix successfully initialized! (LOCAL mode)
echo ======================================================
echo.
echo  Endpoints:
...
echo.
echo !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
echo  [FAIL] Init failed. Rolling back all changes...
echo !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
echo.
